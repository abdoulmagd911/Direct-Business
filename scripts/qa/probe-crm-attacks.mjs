...
async function openApp(BASE, opts = {}) {
  ...
  await tapNotices(p, (m) => dialogs.push({ type: 'alert', message: m }));   // 2026-09-10: alert() is js/63's in-page card now
...
async function phaseA() {
  ...
  const back1 = await p.evaluate(async () => { leadQuickEdit('qa_due'); await new Promise(r => setTimeout(r, 300)); document.getElementById('qe_stage').value = 'Proposal'; document.getElementById('mSave').click(); await new Promise(r => setTimeout(r, 500));
    /* 2026-09-09 (live test D1 family): the question is js/57's in-page box now; Cancel there leaves the form open with nothing saved */
    const box = document.getElementById('pfConfirmBox'); const asked = box ? box.innerText : ''; const no = document.getElementById('pfConfirmNo'); if (no) no.click(); await new Promise(r => setTimeout(r, 300));
    const formOpen = !!document.getElementById('qe_stage'); try { closeModal(); } catch (_) { }
    const b = getLead('qa_due'); return { isClient: b.isClient, stage: leadStage(b), asked, formOpen, native: false }; });
  const backConfirm = dialogs.slice(nDialogs).find(d => d.type === 'confirm');
  check('4c moving a client back from Won asks first (in the page, no native confirm), and Cancel keeps it a client with the form still open', !backConfirm && /currently a client/i.test(back1.asked) && back1.isClient === true && back1.stage === 'Won' && back1.formOpen, { back1, backConfirm });
  ...
  const back2 = await p.evaluate(async () => { leadQuickEdit('qa_due'); await new Promise(r => setTimeout(r, 300)); document.getElementById('qe_stage').value = 'Qualified'; document.getElementById('mSave').click(); await new Promise(r => setTimeout(r, 500)); window.__askedAgain = !!document.getElementById('pfConfirmBox'); const no2 = document.getElementById('pfConfirmNo'); if (no2) no2.click(); try { closeModal(); } catch (_) { } const b = getLead('qa_due'); return { isClient: b.isClient, stage: leadStage(b), asked: false }; });
  const askedAgain = await p.evaluate(() => window.__askedAgain === true);
  ...
  dlg.mode = 'accept';
  const back3 = await p.evaluate(async () => { const b = getLead('qa_due'); b.stage = 'Won'; b.status = 'Won'; leadQuickEdit('qa_due'); await new Promise(r => setTimeout(r, 300)); document.getElementById('qe_stage').value = 'Proposal'; document.getElementById('mSave').click(); await new Promise(r => setTimeout(r, 400)); const yes = document.getElementById('pfConfirmYes'); if (yes) yes.click(); await new Promise(r => setTimeout(r, 500)); const x = getLead('qa_due'); return { isClient: x.isClient, stage: leadStage(x), inLeads: leadTableList().some(y => y.id === 'qa_due') }; });
