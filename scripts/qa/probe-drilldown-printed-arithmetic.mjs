...
async function main() {
  ...
  if (!(await p.waitForFunction(() => typeof window.s1Toggle === 'function' && window.__finSessionOk===true && Array.isArray(FIN.rows) && !FIN.loading && !FIN._waitSignIn, { timeout: 90000 }).then(() => true).catch(() => false)))
