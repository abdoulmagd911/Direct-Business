...
async function boot(role, pageAccess, PORT) {
  ...
  await p.addInitScript(() => { const iv = setInterval(() => { const b = document.getElementById('v70box'); if (b && !b.__seen) { b.__seen = true; try { window.__qaBox(b.innerText.replace(/\s+/g, ' ')); } catch (_) { } } }, 100); });
  ...
  return { state, boxes, errors };
}

async function main() {
  const granted = await boot('team_member', { today: 'editor', leads: 'editor', clients: 'editor', finance: 'editor', ops: 'editor' }, PORT_GRANTED);
  if (granted.state.cur === 'ops' && granted.state.path === '/ops' && !granted.boxes.some((t) => /Not part of your access|خارج نطاق/.test(t))) ok('a team member given Operations boots straight onto /ops — no "Not part of your access" at any point');
