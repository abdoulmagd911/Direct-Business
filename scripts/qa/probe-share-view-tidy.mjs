/* probe-share-view-tidy.mjs — guards js/79: what an OUTSIDER holding a view-only share link sees.
   ...
   visible in the sidebar; current='finance' bounces to Today; footer reads "View-only guest" (and
...
const en = await read();
const bounce = await p.evaluate(() => { try { current = 'finance'; openLead = ''; render(); } catch (_) { } return new Promise((res) => setTimeout(() => res({ current, len: (document.getElementById('view').innerText || '').trim().length }), 1500)); });
...
const checks = [
  ...
  ['landing on Finance in a shared view is sent to Today', bounce.current === 'today'],
