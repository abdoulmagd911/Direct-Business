...
const TABLES={
  ...
  ksa_events:[...Array(8)].map((_,i)=>({id:'e'+i,name_en:'Event '+i,name_ar:'فعالية '+i,vertical:['Travel','Tech','Study','Other'][i%4],status:'confirmed',/* dates are RELATIVE to now (2026-09-14): fixed '2026-09-1x' dates silently ended as the calendar moved, turning probe-events' "7 still ahead" into 4 — a time-bomb the dark battery hid. Event 3 stays the one ended event; the other 7 are always ahead. */start_date:(n=>new Date(Date.now()+n*864e5).toISOString().slice(0,10))(i===3?-30:i+1),end_date:(n=>new Date(Date.now()+n*864e5).toISOString().slice(0,10))(i===3?-29:i+1),city:'Riyadh',venue:'RICEC',organiser:'Org',link:'https://example.com',opportunity_sales:i%2===0,opportunity_partner:i%3===0,priority:(i%5)+1,notes:null,approach:['attend','stand','mine','skip','undecided'][i%5],approach_status:i===0?'signed_up':'not_started',exhibitor_list_url:i===2?'https://example.com/exhibitors':null,created_at:null,updated_at:null})),
