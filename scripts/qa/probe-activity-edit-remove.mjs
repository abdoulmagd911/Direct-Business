...
async function main() {
  ...
  const v = await p.evaluate(() => { window.__pageAccess = { leads: 'viewer' }; window.__pageLevels = { today: 'view', leads: 'view' }; window.__userRole = 'team_member'; render(); return new Promise((res) => setTimeout(() => res({ items: document.querySelectorAll('#view .timeline .tl-item').length, ctrls: document.querySelectorAll('#view [data-act-edit],#view [data-act-remove]').length }), 700)); });
