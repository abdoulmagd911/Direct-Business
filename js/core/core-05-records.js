...
/* 2026-09-02 (round 33): bkMargin was sale − cost with onum(), so a booking whose cost nobody
   has recorded reported its ENTIRE SALE as margin — a 15,500 sale showing 15,500 of profit, and
   the page total carrying it. That is the M8 rule again (cost is approved expenses only), and
   round 29 made it live rather than theoretical: bookings converted from a proposal now
   deliberately carry NO cost instead of the fabricated 85% the old code wrote, so "no cost
   recorded" is the normal shape of a fresh booking, not an oddity.
   A margin with no recorded cost is UNKNOWN. Callers that add margins up must skip those and
   say how many they skipped; callers that print one must print the words, not a number. */
function bkHasCost(b){return !!b&&b.totalCost!=null&&String(b.totalCost).trim()!=='';}
function bkMargin(b){return bkHasCost(b)?(onum(b.totalSale)-onum(b.totalCost)):null;}
function bkCostText(b){return bkHasCost(b)?moneyShort(b.totalCost):((typeof LANG!=='undefined'&&LANG==='ar')?'غير مسجّلة':'not recorded');}
function bkMarginText(b){return bkHasCost(b)?moneyShort(bkMargin(b)):((typeof LANG!=='undefined'&&LANG==='ar')?'غير معروف':'unknown');}
function bkMarginColor(b){return bkHasCost(b)?'#16B364':'#B54708';}
function bkMarginCell(b){return '<span style="color:'+bkMarginColor(b)+'"'+(bkHasCost(b)?'':' title="'+((typeof LANG!=='undefined'&&LANG==='ar')?'لم تُسجَّل تكلفة لهذا الحجز':'No cost has been recorded for this booking')+'"')+'>'+bkMarginText(b)+'</span>';}
/* sum of the margins that are actually evidenced, plus how many bookings were left out */
function bkMarginTotal(list){var L=(list||[]).filter(bkHasCost);return {margin:L.reduce(function(s,b){return s+bkMargin(b);},0),costed:L.length,noCost:(list||[]).length-L.length};}
try{window.bkHasCost=bkHasCost;window.bkMarginText=bkMarginText;window.bkMarginTotal=bkMarginTotal;window.bkMarginCell=bkMarginCell;window.bkCostText=bkCostText;}catch(_){}
...
function renderBookings(v){if(openBooking)return bookingDetail(v,openBooking);
  const B=DB.bookings||[];const sale=B.reduce((s,b)=>s+onum(b.totalSale),0);const _mt=bkMarginTotal(B);const margin=_mt.margin;const tks=B.reduce((s,b)=>s+(b.tickets||[]).length,0);
  v.innerHTML=`
  ${dropZone('booking')}
  <div class="card" style="display:flex;flex-wrap:wrap;gap:18px;padding:14px 20px;margin-bottom:14px"><div><div class="kl">Bookings</div><div class="kv">${B.length}</div></div><div><div class="kl">Tickets</div><div class="kv">${tks}</div></div><div><div class="kl">Total sale</div><div class="kv">${moneyShort(sale)} SAR</div></div><div><div class="kl">Margin</div><div class="kv" style="color:${_mt.costed?'#16B364':'#B54708'}">${_mt.costed?(moneyShort(margin)+' SAR'):'—'}</div>${_mt.noCost?`<div class="bk-nocost-note" style="font-size:10.5px;font-weight:600;color:#B54708;margin-top:2px">${_mt.noCost} with no cost recorded — not counted</div>`:''}</div><div style="flex:1"></div><button class="btn pri" onclick="ingestModal('booking','',null)">+ New booking</button></div>
...
function bookingDetail(v,id){const b=getBooking(id);if(!b){openBooking=null;return renderBookings(v);}
  ...
  <div class="chips" style="margin-bottom:14px"><div class="chip"><div class="v">${moneyShort(b.totalSale)}</div><div class="l">Total sale SAR</div></div><div class="chip"><div class="v">${bkCostText(b)}</div><div class="l">Total cost</div></div><div class="chip"><div class="v" style="color:${bkMarginColor(b)}">${bkMarginText(b)}</div><div class="l">Margin</div></div><div class="chip"><div class="v">${(b.tickets||[]).length}</div><div class="l">Tickets</div></div></div>
...
function invoiceFromBooking(id){const b=getBooking(id);if(!b)return;const inv={id:uid('inv'),number:'INV-'+Date.now().toString().slice(-4),clientId:b.leadId,date:todayISO(),currency:'SAR',status:'Draft',items:[{desc:'Booking '+b.ref+' — '+bkAirlines(b).join(', '),bookingId:b.id,amount:onum(b.totalSale)}],total:onum(b.totalSale),files:[],notes:''};(DB.invoices=DB.invoices||[]).push(inv);b.invoiceId=inv.id;save();openInvoice=inv.id;current='invoices';render();}
...
function editCorporate(id){
  ...
  openModal('Corporate profile — '+esc(b.name),`
    <div class="grid2"><div class="field"><label>Entity type</label><select id="c_et">${/* 2026-09-20 (fire #119) — nineteen live companies carry an entity type — "Small Company", "Government", "Semi Government", "Travel Partner", "Big Company", "Medium Company" — and NOT ONE of them is in ENTITY_TYPES, which offers Ministry, Government entity, Semi-government and so on. Measured live: opening this editor and pressing Save with nothing touched filed a company as a Ministry. A <select> whose option list does not contain the stored value selects its FIRST option, and Save writes that back. Same remedy as the lead, funnel and airline boxes: an empty option for nothing recorded, and the stored value as its own option where the list has no match. */(function(){var _l=ENTITY_TYPES,_c=b.entityType||"",_ar=(typeof LANG!=="undefined"&&LANG==="ar");return '<option value="'+'"'+(_c?"":" selected")+'>'+(_ar?"\u2014 \u063a\u064a\u0631 \u0645\u0633\u062c\u0651\u0644 \u2014":"\u2014 not recorded \u2014")+'</option>'+_l.map(function(o){return '<option'+(_c===o?" selected":"")+'>'+esc(o)+'</option>';}).join('')+((_c&&_l.indexOf(_c)<0)?('<option value="'+esc(_c)+'" selected>'+esc(_c)+' \u00b7 '+(_ar?"\u0627\u0644\u0645\u0633\u062c\u0651\u064e\u0644":"on file")+'</option>'):'');})()}</select></div><div class="field"><label>Account manager</label><input id="c_am" value="${esc(b.accountManager||b.owner||'')}"></div></div>
