...
function supplierDetail(v,kind,id){
  ...
  <div class="detail-grid"><div>
    ...
    <div class="card"><h3>Points of contact</h3>${(x.contacts||[]).length?x.contacts.map(c=>`<div class="contact-row"><div class="ci">${initials(c.name||"?")}</div><div style="flex:1;min-width:0"><div style="font-weight:600">${esc(c.name||'—')}${c.role?` · <span style="color:var(--muted);font-weight:500">${esc(c.role)}</span>`:''}</div><div style="font-size:11.5px;color:var(--muted)">${c.email?`<a href="mailto:${esc(c.email)}">${esc(c.email)}</a>`:''}${c.email&&c.phone?' · ':''}${c.phone?`<a href="${esc(telHref(c.phone))}">${esc(c.phone)}</a> <a href="${esc(waHref(c.phone))}" target="_blank" rel="noopener" style="color:#16B364;font-weight:800" title="WhatsApp">WA</a>`:''}</div></div></div>`).join(""):'<div class="empty">No contacts yet.</div>'}</div>
  ...
  <details class="card"><summary ${sum}>Contacts &amp; notes</summary>
      <div style="margin-top:8px">${(x.contacts||[]).length?x.contacts.map(c=>`<div class="contact-row"><div class="ci">${initials(c.name||"?")}</div><div style="flex:1;min-width:0"><div style="font-weight:600">${esc(c.name||'—')}${c.role?` · <span style="color:var(--muted);font-weight:500">${esc(c.role)}</span>`:''}</div><div style="font-size:11.5px;color:var(--muted)">${c.email?`<a href="mailto:${esc(c.email)}">${esc(c.email)}</a>`:''}${c.email&&c.phone?' · ':''}${c.phone?`<a href="${esc(telHref(c.phone))}">${esc(c.phone)}</a> <a href="${esc(waHref(c.phone))}" target="_blank" rel="noopener" style="color:#16B364;font-weight:800" title="WhatsApp">WA</a>`:''}</div></div></div>`).join(""):'<div class="empty">No contacts yet — add via Edit.</div>'}</div>
...
function editSupplier(kind,id){
  ...
  openModal(isNew?(isAir?"New airline":"New provider"):esc(x.name),`
    ...
    ${isAir?`<div class="grid2"><div class="field"><label>Type</label><select id="x_type">${/* 2026-09-20 (fire #116) — no empty option and no room for anything else, so an airline whose type is blank or is something this box never heard of selected the first entry and Save wrote it back. Five of the 136 real carriers are in that state: two with no type and three that are plating or GSSA platforms — which would have been recorded as full-service carriers. Same remedy as the lead and funnel forms. */(function(){var _c=x.type||"",_l=['FSC','LCC','Charter'],_ar=(typeof LANG!=="undefined"&&LANG==="ar");return '<option value=""'+(_c?"":" selected")+'>'+(_ar?"— غير مسجّل —":"— not recorded —")+'</option>'+_l.map(function(o){return '<option'+(_c===o?" selected":"")+'>'+o+'</option>';}).join('')+((_c&&_l.indexOf(_c)<0)?('<option value="'+esc(_c)+'" selected>'+esc(_c)+' · '+(_ar?"المسجَّل":"on file")+'</option>'):'');})()}</select></div><div class="field"><label>GDS</label><input id="x_gds" value="${esc(x.gds||'')}" placeholder="Amadeus / Sabre / Galileo"></div></div>
...
function editSop(id,whale){const arr=whale?DB.sopsWhale:DB.sops;const isNew=!id;const s=id?JSON.parse(JSON.stringify(arr.find(x=>x.id===id))):{id:uid("s"),code:"SOP",title:"",purpose:"",cmd:"",body:"",market:"",edge:""};
  ...
  ()=>{s.code=val("s_code");s.title=val("s_title");s.purpose=val("s_purpose");s.cmd=val("s_cmd");s.body=val("s_body");s.market=val("s_market");s.edge=val("s_edge");if(!s.title.trim()){toast((typeof LANG!=='undefined'&&LANG==='ar')?'العنوان مطلوب':'Title required','err');const t=document.getElementById('s_title');if(t)t.focus();return false;}const i=arr.findIndex(x=>x.id===s.id);if(i>=0)arr[i]=s;else arr.push(s);save();render();},isNew?null:()=>{const _ar=(typeof LANG!=='undefined'&&LANG==='ar');const go=()=>{const i=arr.findIndex(x=>x.id===id);if(i>=0)arr.splice(i,1);save();render();};const msg=(_ar?'حذف الإجراء «':'Delete the SOP "')+(s.title||'')+(_ar?'»؟\nيُحذف للفريق كله.':'"?\nIt is removed for the whole team.');if(typeof window.pfConfirm==='function')window.pfConfirm(msg,go);else go();});
}
...
function renderSlas(v){
  ...
  ${DB.slas.map(s=>`<tr>
    ...
    <td style="text-align:right;white-space:nowrap"><button class="btn sm ghost" style="color:#a3242c" data-sla-del="${s.id}" onclick="slaDelete('${s.id}')">${(typeof LANG!=='undefined'&&LANG==='ar')?'حذف':'Delete'}</button></td></tr>`).join("")}
  </tbody></table></div></div>
...
/* 2026-09-09 (live test SOP2 + D1): the red ✕ on every row and its native confirm() are gone —
   a plain Delete button that asks through js/57's in-page box. */
function slaDelete(id){const s=DB.slas.find(x=>x.id===id);if(!s)return;const _ar=(typeof LANG!=='undefined'&&LANG==='ar');const go=()=>{DB.slas=DB.slas.filter(x=>x.id!==id);save();render();try{toast(_ar?'حُذف مستوى الخدمة.':'Service level removed.');}catch(_){}};const msg=(_ar?'حذف مستوى الخدمة «':'Delete the service level "')+(s.event||'')+(_ar?'»؟':'"?')+'\n'+(_ar?'يُحذف من الجدول للفريق كله.':'It comes off the table for the whole team.');if(typeof window.pfConfirm==='function')window.pfConfirm(msg,go);else go();}
...
function renderOps(v){
  ...
  /* 2026-09-09 (live test, OPS1) — owner ruling of 21 Aug, verbatim: "any numbers of revenue or
     money or at all, I wanted all to be on the finance page". This board printed "Pipeline value
     183k SAR" and "Booked margin 28k SAR · 15%" (both from practice requests, as it turned out)
     and a "25k SAR · △3k" line on every card. The sell and cost stay where they are ENTERED (the
     request form — an input, like the credit-limit box js/14 keeps); the board reports work,
     not money: how many requests still need a cost recorded is the operational fact that
     replaces the margin tile. The margin arithmetic above is kept for the form and the audit. */
  const kp=[["Open requests",open.length,"var(--blue)"],["SLA overdue",over,"var(--red)"],["Awaiting client",R.filter(r=>r.stage==="Awaiting client").length,"var(--amber)"],[(_arOps?'بلا تكلفة مسجّلة':'Needs a cost recorded'),opsNoCost.length,"var(--violet)"],["Delivered / closed",R.filter(r=>r.stage==="Delivered"||r.stage==="Closed").length,"var(--ink-3)"]];
  v.innerHTML=`
...
function _opsHasCostCard(r){return r&&r.cost!=null&&String(r.cost).trim()!=='';}
function reqCard(r){const over=slaOverdue(r);const early=(r.stage==="New"||r.stage==="Quoting");
  ...
  <div style="display:flex;justify-content:space-between;margin-top:10px;padding-top:9px;border-top:1px solid var(--line);font-size:11.5px;color:var(--muted)"><span>${esc(r.owner||(ar?'غير مُسند':'Unassigned'))}${r.pnr?" · "+esc(r.pnr):""}</span><span style="font-weight:700;color:${r.priority==='Urgent'?'var(--red)':r.priority==='High'?'var(--amber)':'var(--muted)'}">${esc(ar?(prioAr[r.priority]||r.priority||''):(r.priority||""))}${(+r.sell||0)>0&&!_opsHasCostCard(r)?` <span title="${ar?'لم تُسجَّل تكلفة لهذا الطلب بعد':'No cost recorded for this request yet'}" style="color:var(--amber)">· ${ar?'بلا تكلفة':'no cost yet'}</span>`:''}</span></div>${r.stage!=="Closed"?('<button class="btn sm" style="width:100%;margin-top:9px;justify-content:center;padding:6px" onclick="advanceReq(\''+r.id+'\',event)">'+(ar?('التالي &#8592; '+esc(stAr(next))):('Advance &#8594; '+esc(next)+' &#9656;'))+'</button>'):''}</div>`;}
/* The Stage and Priority dropdowns below carry an explicit value="" on every option, added
   2026-09-07 (round 61). They used to be value-less, and js/21's universal rule — an <option>
   with no value attribute is stored BY ITS TEXT, so translating it saves an Arabic word as data —
   correctly refused to touch them. The consequence was that the New-request dialog showed eleven
   English words (New · Quoting · Awaiting client · Booked · Ticketed · Delivered · Closed ·
   Urgent · High · Normal · Low) on an otherwise fully Arabic screen, and no dictionary entry
   could ever have fixed it without corrupting the stored stage. With a value present the stored
   string stays English whatever the label reads, so the words are now translated the same way as
   the kanban column headers they must agree with. */
function editRequest(id){
  ...
  openModal(isNew?"New request":(((typeof LANG!=='undefined'&&LANG==='ar')?"طلب — ":"Request — ")+esc(r.client)),`
  ...
  `,()=>{r.client=val("r_client");r.service=val("r_service");r.stage=val("r_stage");r.detail=val("r_detail");r.owner=val("r_owner");r.priority=val("r_priority");r.supplier=val("r_supplier");r.pnr=val("r_pnr");r.sell=+val("r_sell")||0;r.cost=+val("r_cost")||0;r.notes=val("r_notes");if(!r.client.trim()){try{var _rc=document.getElementById("r_client");if(_rc){_rc.style.borderColor="#D92D20";_rc.focus();}}catch(_){}var _m=(typeof LANG!=="undefined"&&LANG==="ar")?"العميل مطلوب.":"Client required.";if(typeof toast==="function")toast(_m,"err");else alert(_m);return false;}DB.requests=DB.requests||[];const i=DB.requests.findIndex(x=>x.id===r.id);if(i>=0)DB.requests[i]=r;else DB.requests.push(r);save();render();},isNew?null:()=>{const _doDel=()=>{DB.requests=DB.requests.filter(x=>x.id!==r.id);save();render();};const _q=(typeof LANG!=="undefined"&&LANG==="ar")?"حذف هذا الطلب؟":"Delete this request?";if(window.pfConfirm)pfConfirm(_q,_doDel);else if(confirm(_q))_doDel();});
}
