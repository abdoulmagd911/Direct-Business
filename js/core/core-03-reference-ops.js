...
function editRequest(id){
  ...
  openModal(isNew?"New request":"Request — "+esc(r.client),`
    ...
    <div class="grid2"><div class="field"><label>Service</label><select id="r_service">${["Flights","Hotels","Transfer / Chauffeur","Visa","eSIM","Package","Group / MICE","Other"].map(s=>`<option ${r.service===s?"selected":""}>${s}</option>`).join("")}</select></div><div class="field"><label>Stage</label><select id="r_stage">${STAGES.map(s=>`<option ${r.stage===s?"selected":""}>${s}</option>`).join("")}</select></div></div>
