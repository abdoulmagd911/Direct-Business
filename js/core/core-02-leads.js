...
board.innerHTML=`<div class="chips" style="margin-bottom:14px"><div class="chip"><div class="v">${B.length}</div><div class="l">Total leads</div></div><div class="chip"><div class="v">${open.length}</div><div class="l">In pipeline</div></div><div class="chip"><div class="v">${clients}</div><div class="l">Became client</div></div><div class="chip"><div class="v">${lost}</div><div class="l">Lost</div></div></div>
...
function leadDashboard(v,id){
  ...
  <div class="detail-grid"><div>
    ...
    <div class="card"><h3>Contacts &amp; channels</h3>${(b.contacts||[]).length?b.contacts.map(c=>`<div class="contact-row"><div class="ci">${initials(c.name||c.email||"?")}</div><div style="flex:1;min-width:0"><div style="font-weight:600">${esc(c.name||"—")}</div><div style="font-size:11.5px;color:var(--muted)">${c.email?esc(c.email):""}${c.email&&c.phone?" · ":""}${c.phone?esc(c.phone):""}</div></div></div>`).join(""):'<div class="empty">No contacts yet.</div>'}${(b.channels||[]).length?`<div style="margin-top:8px;font-size:12px"><span style="color:var(--muted)">Channels: </span>${b.channels.map(esc).join(", ")}</div>`:''}</div>
  </div></div>`;
}
function renderLeadDetail(v,id){
  ...
  <div class="detail-grid">
    <div>
      ...
      <div class="card"><h3>Contacts</h3>${(b.contacts||[]).length?b.contacts.map(c=>`<div class="contact-row"><div class="ci">${initials(c.name||c.email||"?")}</div><div style="flex:1;min-width:0"><div style="font-weight:600">${esc(c.name||"—")}</div><div style="font-size:11.5px;color:var(--muted)">${c.email?`<a href="mailto:${esc(c.email)}">${esc(c.email)}</a>`:""}${c.email&&c.phone?" · ":""}${c.phone?`<a href="tel:${esc(c.phone)}">${esc(c.phone)}</a> <a href="https://wa.me/${String(c.phone).replace(/[^0-9]/g,"").replace(/^0/,"966")}" target="_blank" rel="noopener" style="color:#16B364;font-weight:800" title="WhatsApp">WA</a>`:""}</div></div></div>`).join(""):'<div class="empty">No contacts yet.</div>'}</div>${corpCard(b)}
    </div>
    <div>
      <div class="card"><h3>Key facts</h3>
        <div class="fact"><span class="k">Lifetime billed</span><span class="v">${money(b.totalSAR)}</span></div>
        <div class="fact"><span class="k">Invoices</span><span class="v">${b.invoices||0}</span></div>
        <div class="fact"><span class="k">Category</span><span class="v">${b.category}</span></div>
...
function renderClients(v){
  ...
  <div class="card" style="display:flex;flex-wrap:wrap;gap:18px;align-items:center;padding:14px 20px;margin-bottom:14px">
    <div><div class="kl">Clients in view</div><div class="kv">${cl.length}</div></div>
    <div><div class="kl">Key accounts</div><div class="kv">${cl.filter(b=>b.tier==="Key").length}</div></div>
    <div><div class="kl">Billed (in view)</div><div class="kv">${moneyShort(total)} SAR</div></div>
