...
/* 2026-09-18 (fire #94): every price on a quotation used to be printed with a bare
   toLocaleString call with no language named — so the EMPLOYEE'S BROWSER decided the digits, not
   the app's own language switch. Driven for real: in a browser set to Arabic, the option totals and
   the per-passenger figure came out "١٬٢٣٤٬٥٦٧٫٥" while the headline Total on the same quotation
   stayed "1,234,567.5", because that one is printed straight from what was typed. One price
   document, two number systems, decided by whose laptop it happened to be opened on — and the same
   split reached the WhatsApp text an agent copies out to a client.
   Every other money figure in this app is printed with 'en-US' (Finance, the ledger, the importer),
   Saudi price documents use Western digits, and the headline total does too. So this says 'en-US'
   once, out loud, and the whole document agrees with itself on every machine.
   The quotation's VAT column is untouched and stays: VAT is legally expected on a client-facing
   document — M1 forbids it entering cost, profit or revenue, which is a different thing. */
function omoney(n){try{return (Number(n)||0).toLocaleString('en-US');}catch(_){return String(n);}}
...
/* 2026-09-02 (round 29) — the proposal margin broke both money rules at once.
   ...
   ticket + partner fees + service fees + VAT + DIP — so **VAT sat inside the margin**
   (M1: VAT never enters cost, profit or revenue). And a blank cost read as 0 through onum(),
   so an unpriced proposal printed its WHOLE sale as profit (M8: never fabricate a number to
   fill a gap; leave it unknown and say why).
   Now: the sale is taken net of VAT, and a margin with no recorded cost is UNKNOWN (null) —
   every caller must say "cost not recorded" instead of printing a number. */
function offerNet(o){return onum(o.total)-onum(o.vat);}
function offerHasCost(o){return String((o&&o.cost!=null)?o.cost:'').trim()!=='';}
function offerMargin(o){return offerHasCost(o)?(offerNet(o)-onum(o.cost)):null;}
function offerMarginText(o){
  var m=offerMargin(o), ar=(typeof LANG!=='undefined'&&LANG==='ar');
  if(m===null)return ar?'التكلفة غير مسجّلة':'Cost not recorded';
  return omoney(m)+' '+((o&&o.currency)||'');
}
...
function o_logToLead(){const o=curOffer();if(!o)return;if(!o.linkedLeadId){alert((typeof LANG!=='undefined'&&LANG==='ar')?'اربط عميلًا محتملًا أو عميلًا أولًا (قسم الصفقة وسير العمل).':'Link a lead/client first (Deal & workflow section).');return;}const b=getLead(o.linkedLeadId);if(!b){alert((typeof LANG!=='undefined'&&LANG==='ar')?'العميل المحتمل المرتبط غير موجود.':'Linked lead not found.');return;}b.activities=b.activities||[];b.activities.push({date:Date.now(),type:'Offer',status:'',note:'Offer '+(o.ref||'')+' ('+(o.status||'Draft')+')'+(o.subject?' — '+o.subject:'')+(o.total?(' · '+o.total+' '+(o.currency||'')):''),by:o.owner||'Abdelrahman'});b.lastContact=Date.now();save();alert('Logged to '+b.name+'’s work log.');}
...
function drawOffers(q){q=(q||'').toLowerCase();const ar=(typeof LANG!=='undefined'&&LANG==='ar');const tb=document.getElementById('otb');if(!tb)return;const _meO=(window.meName?meName():'');const O=(DB.offers||[]).filter(o=>(!q||((o.ref||'')+' '+(o.client||'')+' '+(o.subject||'')+' '+(o.proposalType||'')+' '+(o.airline||'')).toLowerCase().includes(q))&&(!window.offerMine||(window.sameOwner?sameOwner(o.owner,_meO):(o.owner||'')===_meO)));
...
function offerEditor(v,id){
  ...
  <div class="offer-wrap">
   <div class="offer-form card"><h3>${_pl('Quote details (travel)','تفاصيل عرض السفر')}</h3>
     ...
     <div style="margin:-4px 0 8px"><select onchange="o_loadClient(this.value)" style="width:100%;padding:8px 10px;border:1px solid var(--line-2);border-radius:9px;font:inherit;font-size:12.5px"><option value="">${_pl("↧ Load a corporate client's negotiated deal & pricing…","↧ تحميل اتفاقية عميل مؤسسي وأسعاره المتفق عليها…")}</option>${DB.businesses.filter(x=>x.isClient||((x.pricing||[]).length)||((x.airlineDeals||[]).length)).map(x=>'<option value="'+x.id+'">'+esc(x.name)+'</option>').join('')}</select></div>
     ...
     <div style="margin-top:8px;padding:9px 11px;background:#f6f3ee;border-radius:9px;font-size:12.5px">🔒 <b>${_pl('Agency only:','للوكالة فقط:')}</b> ${_pl('Cost','التكلفة')} <input value="${esc(o.cost||'')}" oninput="o_set('cost',this.value);var _m=document.getElementById('mgn');if(_m){_m.textContent=offerMarginText(curOffer());_m.style.color=offerMarginColor(curOffer());}" style="width:78px;border:1px solid var(--line-2);border-radius:6px;padding:3px 6px;font:inherit"> · ${_pl('Commission','العمولة')} <input value="${esc(o.commission||'')}" oninput="o_set('commission',this.value)" style="width:78px;border:1px solid var(--line-2);border-radius:6px;padding:3px 6px;font:inherit"> · <b>${_pl('Margin:','الهامش:')} <span id="mgn" style="color:${offerMarginColor(o)}">${esc(offerMarginText(o))}</span></b>${(offerFreebieCost(o)&&offerMargin(o)!==null)?` · 🎁 ${_pl('freebies cost','تكلفة الهدايا')} ${omoney(offerFreebieCost(o))} · <b>${_pl('net','الصافي')} ${omoney(offerMargin(o)-offerFreebieCost(o))}</b>`:(offerFreebieCost(o)?` · 🎁 ${_pl('freebies cost','تكلفة الهدايا')} ${omoney(offerFreebieCost(o))}`:'')}</div>
     <details ${(window.__ofO=window.__ofO||{}).opt?'open':''} ontoggle="__ofO.opt=this.open"><summary style="cursor:pointer;font-weight:800;font-size:14px;margin-top:14px;color:var(--ink)">${_pl('Fare options — compare 2–3 fares','خيارات الأسعار — قارن 2–3 أسعار')} (${(o.options||[]).length})</summary>
...
function offerHTML(o){const cur=esc(o.currency||'SAR');const c=v=>esc(v||'—');
  ...
  ${offerPax(o)>1?`<div class="odoc-row" style="font-size:12px;color:#555">Total for ${offerPax(o)} pax · ≈ ${omoney(Math.round(onum(o.total)/offerPax(o)))} ${cur}/pax (${o.paxAdt||0} ADT · ${o.paxChd||0} CHD · ${o.paxInf||0} INF)</div>`:''}
  ${(o.options&&o.options.length)?`<div style="font-weight:700;margin:12px 0 4px;color:var(--ink)">Compare fare options</div><table class="odoc-tbl"><thead><tr><th>Option</th><th>Source</th><th>Base</th><th>Taxes</th><th>Anc.</th><th>Fee</th><th>Total</th><th>Refund</th><th>Baggage</th><th>Includes</th></tr></thead><tbody>${o.options.map(op=>`<tr><td><b>${c(op.label)}</b>${op.fareFamily?'<br><span style="font-size:11px;color:#888">'+c(op.fareFamily)+'</span>':''}</td><td>${c(op.provider)}${op.content&&op.content!=='—'?' <span style="font-size:10px;background:#eef;color:#436;padding:1px 5px;border-radius:6px">'+c(op.content)+'</span>':''}</td><td>${c(op.base)}</td><td>${c(op.taxes)}</td><td>${c(op.anc)}</td><td>${c(op.fee)}</td><td style="font-weight:800;color:var(--orange)">${omoney(optTotal(op))} ${cur}</td><td>${c(op.refundable)}</td><td>${c(op.baggage)}</td><td style="font-size:11px">${[...(op.items||[]).map(it=>esc(it.type)+(it.detail?' '+esc(it.detail):'')+(it.price?' ('+esc(it.price)+')':'')),...(op.freebies||[]).map(fb=>'🎁 '+esc(fb.label||'')+' (free)')].join('<br>')||'—'}</td></tr>`).join('')}</tbody></table>`:''}
...
function offerText(o){const c=o.currency||'SAR';const L=[];
  ...
  L.push('*Total: '+(o.total||'')+' '+c+'*');if(offerPax(o)>1)L.push('('+offerPax(o)+' pax · '+omoney(Math.round(onum(o.total)/offerPax(o)))+' '+c+'/pax)');
  if(o.options&&o.options.length){L.push('');L.push('— Compare options —');o.options.forEach(function(op){L.push('• '+(op.label||'')+(op.provider?(' ['+op.provider+(op.content&&op.content!=='—'?'/'+op.content:'')+']'):'')+': '+omoney(optTotal(op))+' '+c+(op.refundable?(' · '+op.refundable+' refund'):''));});}
