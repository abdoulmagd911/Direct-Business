...
function offerEditor(v,id){
  ...
  <div class="offer-wrap">
   <div class="offer-form card"><h3>${_pl('Quote details (travel)','تفاصيل عرض السفر')}</h3>
     ...
     <div style="margin-top:8px;padding:9px 11px;background:#f6f3ee;border-radius:9px;font-size:12.5px">🔒 <b>Agency only:</b> Cost <input value="${esc(o.cost||'')}" oninput="o_set('cost',this.value);var _m=document.getElementById('mgn');if(_m){_m.textContent=offerMarginText(curOffer());_m.style.color=offerMarginColor(curOffer());}" style="width:78px;border:1px solid var(--line-2);border-radius:6px;padding:3px 6px;font:inherit"> · Commission <input value="${esc(o.commission||'')}" oninput="o_set('commission',this.value)" style="width:78px;border:1px solid var(--line-2);border-radius:6px;padding:3px 6px;font:inherit"> · <b>Margin: <span id="mgn" style="color:${offerMarginColor(o)}">${esc(offerMarginText(o))}</span></b>${(offerFreebieCost(o)&&offerMargin(o)!==null)?` · 🎁 freebies cost ${offerFreebieCost(o).toLocaleString()} · <b>net ${(offerMargin(o)-offerFreebieCost(o)).toLocaleString()}</b>`:(offerFreebieCost(o)?` · 🎁 freebies cost ${offerFreebieCost(o).toLocaleString()}`:'')}</div>
