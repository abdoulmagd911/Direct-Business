...
/* 2026-09-10 (second live pass, Settings) — RETIRED. The workspace went live on 22 Aug. This
   button (visible to every admin and manager) dropped every company that was not a client from
   DB.businesses, cleared the activities, deal value and wallet of the ones it kept, emptied
   offers, bookings, invoices, expenses, refunds and the audit, and saved — so the next push
   archived every lead and prospect in the table. Its promised safety net was empty: the
   "Pre-go-live snapshot" it wrote first was localStorage's copy of the workspace, which js/02
   removes on every load in the cloud era, so the snapshot's data was ''. The tool now says so
   and does nothing; the original is in git history (before this commit) if it is ever wanted. */
function v22ResetForGoLive(){var _ar=(typeof LANG!=='undefined'&&LANG==='ar');
  alert(_ar?'أُوقفت أداة «إعادة الضبط للتشغيل» منذ التشغيل الفعلي في 22 أغسطس: كانت تؤرشف كل عميل محتمل وتمسح العروض والحجوزات والفواتير. لم يتغيّر شيء.'
    :'"Reset for go-live" is retired since the workspace went live on 22 Aug: it archived every lead and wiped offers, bookings, invoices and expenses. Nothing was changed.');}
...
function v22OpenClientOnboarding(id){const c=getLead(id);if(!c)return;openModal('🏛 Client onboarding — '+esc(c.name),`<div style="font-size:12.5px;line-height:1.55">
  <details class="v19-disclose" open><summary>Identity (EN + AR)</summary><div class="ds-body">
    ...
    <div class="grid2"><div class="field"><label>Classification</label><select id="v22_class">${/* 2026-09-20 (fire #119) — the same box as the corporate editor's entity type, in the onboarding form. A <select> whose option list does not contain the stored value selects its FIRST option, and Save writes that back. Same remedy as the lead, funnel and airline boxes: an empty option for nothing recorded, and the stored value as its own option where the list has no match. */(function(){var _l=V22_CLASSIFICATIONS,_c=c.classification||"",_ar=(typeof LANG!=="undefined"&&LANG==="ar");return '<option value="'+'"'+(_c?"":" selected")+'>'+(_ar?"\u2014 \u063a\u064a\u0631 \u0645\u0633\u062c\u0651\u0644 \u2014":"\u2014 not recorded \u2014")+'</option>'+_l.map(function(o){return '<option'+(_c===o?" selected":"")+'>'+esc(o)+'</option>';}).join('')+((_c&&_l.indexOf(_c)<0)?('<option value="'+esc(_c)+'" selected>'+esc(_c)+' \u00b7 '+(_ar?"\u0627\u0644\u0645\u0633\u062c\u0651\u064e\u0644":"on file")+'</option>'):'');})()}</select></div><div class="field"><label>Industry</label><input id="v22_ind" value="${esc(c.industry||'')}"></div></div>
  ...
  <details class="v19-disclose"><summary>Pricing scheme + Payment scheme</summary><div class="ds-body">
    <div class="grid2"><div class="field"><label>Pricing scheme</label><select id="v22_ps">${/* 2026-09-20 (fire #119) — a pricing scheme recorded outside this list would be replaced by the first one on Save. A <select> whose option list does not contain the stored value selects its FIRST option, and Save writes that back. Same remedy as the lead, funnel and airline boxes: an empty option for nothing recorded, and the stored value as its own option where the list has no match. */(function(){var _l=V22_PRICING_SCHEMES,_c=c.pricingScheme||"",_ar=(typeof LANG!=="undefined"&&LANG==="ar");return '<option value="'+'"'+(_c?"":" selected")+'>'+(_ar?"\u2014 \u063a\u064a\u0631 \u0645\u0633\u062c\u0651\u0644 \u2014":"\u2014 not recorded \u2014")+'</option>'+_l.map(function(o){return '<option'+(_c===o?" selected":"")+'>'+esc(o)+'</option>';}).join('')+((_c&&_l.indexOf(_c)<0)?('<option value="'+esc(_c)+'" selected>'+esc(_c)+' \u00b7 '+(_ar?"\u0627\u0644\u0645\u0633\u062c\u0651\u064e\u0644":"on file")+'</option>'):'');})()}</select></div><div class="field"><label>Minimum volume commitment</label><input id="v22_mv" value="${esc(c.minVolumeCommit||'')}" placeholder="e.g. 50,000 SAR/mo"></div></div>
    <div class="grid2"><div class="field"><label>Payment configuration</label><select id="v22_pc">${/* 2026-09-20 (fire #119) — a payment configuration written by an import — "Post-paid · Monthly · 30 days" is how the live records spell it — is not one of these three. A <select> whose option list does not contain the stored value selects its FIRST option, and Save writes that back. Same remedy as the lead, funnel and airline boxes: an empty option for nothing recorded, and the stored value as its own option where the list has no match. */(function(){var _l=['PostPaid','PrePaid','Tender'],_c=c.paymentConfiguration||"",_ar=(typeof LANG!=="undefined"&&LANG==="ar");return '<option value="'+'"'+(_c?"":" selected")+'>'+(_ar?"\u2014 \u063a\u064a\u0631 \u0645\u0633\u062c\u0651\u0644 \u2014":"\u2014 not recorded \u2014")+'</option>'+_l.map(function(o){return '<option'+(_c===o?" selected":"")+'>'+esc(o)+'</option>';}).join('')+((_c&&_l.indexOf(_c)<0)?('<option value="'+esc(_c)+'" selected>'+esc(_c)+' \u00b7 '+(_ar?"\u0627\u0644\u0645\u0633\u062c\u0651\u064e\u0644":"on file")+'</option>'):'');})()}</select></div><div class="field"><label>Credit limit (SAR)</label><input id="v22_cl" type="number" value="${c.creditLimit||0}"></div></div>
...
function v22WorkflowSuite(){V22_SUITE.tests=[];V22_SUITE.startedAt=Date.now();
  ...
  // ---- TEST 8: Expenses ----
  DB.expenses=DB.expenses||[];const exp1={id:'exp_v22a_'+Date.now().toString(36),bookingId:testBooking.id,invoiceId:testInvoice.id,amount:Math.round(testBooking.totalCost*0.85),currency:'SAR',date:todayISO(),fop:'IATA BSP via Amadeus',txnId:'BSP-'+Date.now().toString(36).toUpperCase(),proofs:[{name:'BSP_settlement_2026-06.pdf',at:Date.now()}],status:'Approved',[testTag]:true};const exp2={id:'exp_v22b_'+Date.now().toString(36),bookingId:testBooking.id,invoiceId:testInvoice.id,amount:Math.round(testBooking.totalCost*0.15),currency:'SAR',date:todayISO(),fop:'Virtual Credit Card',cardId:'Direct VCC 4242',txnId:'VCC-'+Date.now().toString(36).toUpperCase(),proofs:[{name:'VCC_statement.pdf',at:Date.now()}],status:'Approved',[testTag]:true};DB.expenses.push(exp1,exp2);
  ...
  testInvoice.status='Paid';testInvoice.payments=[{date:todayISO(),amount:totals.total,method:'Bank transfer',ref:'PAY-'+Date.now().toString(36)}];
...
const _v22OrigRender=render;render=function(){_v22OrigRender();setTimeout(()=>{const view=document.getElementById('view');if(!view)return;// Settings v22 additions
  if(current==='settings'&&!view.querySelector('.v22SettingsCard')){const card=document.createElement('div');card.className='card v22SettingsCard';card.innerHTML=`<h3>🚀 v22 — Workflow + go-live</h3><div class="ch-sub" data-v22-retired>${(typeof LANG!=='undefined'&&LANG==='ar')?'أُوقفت منذ التشغيل الفعلي (22 أغسطس): كانت مجموعة الاختبار تكتب بيانات تجريبية في مساحة العمل الحية، وكانت «إعادة الضبط للتشغيل» تؤرشف كل عميل محتمل. تعملان في بيئة الاختبار فقط.':'Retired since go-live (22 Aug): the test suite wrote practice records into the live workspace, and "Reset for go-live" archived every lead. Both run in the QA harness only.'}</div><div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:10px"><button class="btn sm" onclick="v22WipeWorkflowTestData()">🧹 Wipe v22 test data</button></div></div>`;view.appendChild(card);}
...
function v23BankRefund(invoiceId,amount,note){const i=getInvoice(invoiceId);if(!i)return false;i.payments=i.payments||[];i.payments.push({date:todayISO(),amount:-Math.abs(+amount||0),method:'Direct Payment refund (bank)',ref:'rfd-'+Date.now().toString(36)});if(typeof logAudit==='function')logAudit('invoice',invoiceId,'bank-refund','-'+(+amount||0)+' '+(note||''));return true;}
