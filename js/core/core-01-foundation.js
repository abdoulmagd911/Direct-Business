...
const SEED = {
  ...
  vendors:[
    ...
    {id:"v_zetexa",name:"Zetexa",type:"eSIM",source:"White-label eSIM platform / API",portal:"Zetexa partner portal",adminPortal:"",login:"partner account",payment:"wallet / card",terms:"White-label eSIM",sla:"—",adm:"",process:"Issue eSIM via white-label; deliver QR to client.",contact:"",contacts:[{name:"Varun",role:"Partnerships (MENA)",email:"",phone:""}],notes:"Airalo/Simly also evaluated."},
...
function importData(inp){const f=inp.files[0];if(!f)return;const r=new FileReader();r.onload=()=>{let d;try{d=JSON.parse(r.result);if(!d||typeof d!=="object"||Array.isArray(d))throw new Error("not a workspace file");}catch(e){alert((typeof LANG!=='undefined'&&LANG==='ar')?'ملف غير صالح.':"Invalid file.");return;}askInPage("Import this file? Settings and records are replaced by the file's. Leads and clients are NOT imported — they stay exactly as they are.",function(){const _kb=Array.isArray(DB.businesses)?DB.businesses:[];DB=d;DB.businesses=_kb;save();render();alert((typeof LANG!=='undefined'&&LANG==='ar')?'تم الاستيراد — لم تُمسّ بيانات العملاء المحتملين والعملاء.':"Imported — leads and clients untouched.");});try{inp.value="";}catch(_){}};r.readAsText(f);}
