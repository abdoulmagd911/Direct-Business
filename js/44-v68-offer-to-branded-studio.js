/* ===== v68: Offer Builder → branded Proposal Studio bridge =====
   ...
   client-facing document. Renamed from v47 → v68 (v47 is taken). */
(function(){try{
  ...
  window.v68StudioHandoff=function(offerId){
    try{
    ...
    }catch(e){ if(window.console)console.warn('[v68] handoff',e); }
  };
  ...
  function labelPlainExports(ar){
    try{
      ...
      for(var i=0;i<btns.length;i++){
        ...
        if(b.getAttribute('data-v68lbl')) continue;
        if(oc.indexOf('o_download()')>=0){
          b.setAttribute('data-v68lbl','1');
  ...
  function inject(){
    try{
      ...
      if(!id){ return; }
      if(document.getElementById('v68StudioBtn')) return;
      ...
      b.onclick=function(){ window.v68StudioHandoff(id); };
  ...
  console.info('%c[v68] offer→studio bridge loaded','color:#F47A1F;font-weight:700');
